...
lemma p_eq_q {p q : Nat} (hp : p.Prime) (hq : q.Prime) (hdvd : q ∣ p) : p = q := by
  have q_ne_1 : q ≠ 1 := hq.ne_one
  exact (Nat.Prime.dvd_iff_eq hp q_ne_1).mp hdvd

lemma p_values {p : Nat} (hp : p.Prime) (hdvd : 3 ∣ p ∨ 5 ∣ p ∨ 7 ∣ p ∨ 13 ∣ p ∨ 17 ∣ p ∨ 241 ∣ p) :
...
lemma pow2_mod_periodic (k p d : Nat) (h : 2 ^ d ≡ 1 [MOD p]) (hd : 0 < d) :
  2^k ≡ 2^(k % d) [MOD p] := by
  let r := k % d; let q := k / d
...
theorem values_exclusion (n k p : Nat)
    (h_rep : n = p + 2 ^ k)
    (hn7 : n ≡ 1 [MOD 7])
    (hn3 : n ≡ 1 [MOD 3])
    (hp : p.Prime)
  ...
  · -- p = 3
    ...
    (have h_c := h.trans (Nat.ModEq.add (Nat.ModEq.refl 3) hk)
...
theorem exists_progression_no_prime_add_pow_two (hn : n ≡ a [MOD m]) :
  n ≡ a [MOD m] → ¬ ∃ (p k : ℕ), p.Prime ∧ n = p + 2^k := by sorry
